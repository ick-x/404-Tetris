...
document.addEventListener('DOMContentLoaded', () => {
        ...
        function updateV2(coef1, coef2, coef3, coef4) {
            displayScore.innerHTML = "Score : " + score;
            ...
            if (currentPiece) {
                ...
                if (collision()) {
                    ...
                    drawNextPiece()
                    treeSearch = new TreeSearchIA([buildPiece(currentPiece), buildPiece(nextPiece)], tetrisGrid, currentPiece, new GridEvaluator(coef1, coef2, coef3, coef4))
                } else {
                    treeSearch.updateTetris(currentPiece, tetrisGrid)
                }
            }
        ...
        function gameLoopV2(coef1, coef2, coef3, coef4) {
            if (!updateV2(coef1, coef2, coef3, coef4)) {
                return score;
            }
            drawGridAndPiece();
            refreshScore(lineCount);
            return gameLoopV2(coef1, coef2, coef3, coef4)
        }

        function resetTetrisData(coef1, coef2, coef3, coef4) {
            newGrid()
        ...
        const NB_PART = 10
        function gameLoopAndGetScore(coef1, coef2, coef3, coef4) {
            let score = 0
            for (let i = 0; i < NB_PART; ++i) {
                resetTetrisData(coef1, coef2, coef3, coef4)
                score += gameLoopV2(coef1, coef2, coef3, coef4)
                console.log("lost")
            }
        }

        gameLoopAndGetScore(0,0,0,0)
    }
)
;
